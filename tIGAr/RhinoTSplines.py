...
# compute global basis functions at point xi = (u,v), with extraction operator
# C, in which each row specifies a linear combination of Bezier shape functions
...
# get everything covered by one coordinate chart, space out elements so
# that element i goes from 3.0*i to 3.0*i+2.0 in x0 direction.  Thus, if
# we round down (x0/3.0 + EPS), it returns the element index, and we have
# u = x0 - 3.0*i - 1.0, v = x1 for eval-ing shape functions
...
class RhinoTSplineControlMesh(AbstractControlMesh):
    ...
    def __init__(self,fname):
        ...
        # TODO: check w/ ming-chen/fei to se what the true Rhino format is
        lines = fs.split("\n")
